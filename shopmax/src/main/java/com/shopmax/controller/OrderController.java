...
public class OrderController {
    ...
    public @ResponseBody ResponseEntity order(@RequestBody @Valid OrderDto orderDto, // OrderDto는 ajax에서 전달해준 데이터를 받아온다.
                                              //http통신에서 body에 담겨져서 오므로 반드시 @RequestBody 어노테인션 추가.
        ...
        if(bindingResult.hasErrors()) {
            ...
            for (FieldError fieldError : fieldErrors) {
                sb.append(fieldError.getDefaultMessage()); // 에러 메세지를 가지고온다.
            }
 ...
 //Principal: 로그인한 사용자의 정보를 가져온다.
   ...
   @PatchMapping("/order/{orderId}/cancel")
    public @ResponseBody ResponseEntity cancelOrder(@PathVariable("orderId") Long orderId, Principal prdincipal) {

        //1. 주문취소 권한이 있는지 확인(본인확인)
       if(!orderService.validateOrder(orderId, prdincipal.getName())) {
