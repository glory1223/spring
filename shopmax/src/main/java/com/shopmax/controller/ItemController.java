...
public class ItemController {
    ...
    public String itemDtl(@PathVariable("itemId") Long itemId, Model model) {
        try {
           ...
           model.addAttribute("itemFormDto" , itemFormDto);
            ...
            // 에러발생시 비어있는 객체를 넘겨준다 -> 수정화면에서 itemFormDto를 사용하고 있으므로
            model.addAttribute("itemFormDto", new ItemFormDto());
            return "item/itemForm"; // 등록화면으로 다시 이동.
        }
    ...
    public String itemUpdate(@Valid ItemFormDto itemFormDto,  BindingResult bindingResult, Model model, List<MultipartFile> itemImgFileList) {
        if(bindingResult.hasErrors()) return "item/itemForm"; // 유효성  체크에서 걸리면
        ...
        try {
            ...
            e.printStackTrace();
            model.addAttribute("errorMessage", "상품 수정중 에러가 발생헀습니다.");
            return "item/itemForm";
        }
        return "redirect:/";
    }
    */
    ...
    public String itemManage(ItemSearchDto itemSearchDto, @PathVariable("page")Optional<Integer> page, Model model) { //optional객체이기때문에 그냥 가져오면 안되고get으로 가져옴.

        //pageRequest.of(페이지 번호, 한페이지당 조회할 데이터 갯수)
